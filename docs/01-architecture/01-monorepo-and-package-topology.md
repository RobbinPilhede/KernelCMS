...
- **`@kernel/ui` is data-blind.** It imports config _types_ from core but never the operation core, never `@kernel/client`, never a query. Components receive values and callbacks as props so the kit stays portable and Storybook-able. Contrast Sanity, where studio input components are wired into the content store.
