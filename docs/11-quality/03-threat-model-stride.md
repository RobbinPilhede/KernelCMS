...
- **Boundary A — network edge.** Every byte from a browser or API client is hostile. The Local API runs in-process and skips the wire, but it does *not* skip access control — same operation core, same checks.
  ...
  access: {
    read: ({ req }) =>
      req.user?.role === 'editor'
        ? true
        : { status: { equals: 'published' } }, // returns a where-filter, not a boolean
