...
`access` is a set of functions, one per operation, each returning a `boolean` or a **query constraint**. Returning a constraint is the feature that makes row-level authorization ergonomic: instead of "can this user read posts?" you express "this user can read _these_ posts," and the constraint is merged into the `where` clause of every query across REST, GraphQL, and Local API.
