...
- **Collection access** (create/read/update/delete) ✅. **Field access** (read/
  create/update) 🟡 — we enforce _write_ field access; _read_ stripping not yet. P0/M.
...
- **First-run setup** ✅ (takeover-proof) — _ahead of Payload_.
...
- **Build:** a versions store (separate table per collection), draft/published
  status + access integration, autosave, version diff UI (ours can diff _rendered
  sections_ — a differentiator), restore, and scheduled publish via a job.
...
- **The panel** ✅ (ours is React/TanStack, framework-agnostic — _differentiator_).
...
- **MCP plugin** (expose the CMS to AI agents) ❌. P2 — _natural fit given our stack;
  could leapfrog by making MCP first-class._
...
- **Folders** — explicitly _beta_; being stabilized. (Content organization push.)
...
- **TypeScript plugin** — _experimental_; tighter type ergonomics in progress.
...
- Live preview useful _before_ a frontend exists (built-in renderer).
